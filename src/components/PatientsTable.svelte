<script lang="ts">
	import type { Patient } from '$lib/store/types.js';
	import { loadPatients as storeLoadPatients, savePatient, updatePatient, deletePatient } from '$lib/store/patients.js';
	...
	$effect(() => {
		loadSurgeons()
			.then(surgeons => { surgeonsList = surgeons; })
			.catch(() => {});
	});
	...
	async function loadPatientsData() {
		...
		error = '';
		try {
			patients = await storeLoadPatients(destHandle);
		} catch (e) {
	...
	async function saveEdit() {
		...
		try {
			await updatePatient(editingCase, editForm, destHandle);
	...
	async function submitAdd() {
		...
		try {
			await savePatient(newPatient, destHandle);
			await loadPatientsData();
			cancelAdd();
		} catch (e) {
	...
	async function executeDelete() {
		...
		try {
			await deletePatient(deleteTarget.case_number, destHandle);
			await loadPatientsData();
			cancelDelete();
		} catch (e) {
