import { loadSettings } from '$lib/store/settings.js';
import { loadProcedures } from '$lib/store/procedures.js';
import { loadSurgeons } from '$lib/store/surgeons.js';
import { searchPatients } from '$lib/store/patients.js';
import { sortImage } from '$lib/store/sort-image.js';
import type { SortFormData } from '$lib/store/types.js';
...
export function createCaseNumberInputState(props: CaseNumberInputProps) {
	...
	let hasImage = $derived(props.selectedFilename() !== '');
	...
	$effect(() => {
		if (caseNumber.length >= 1) {
			...
			searchTimeout = setTimeout(async () => {
				try {
					const results = await searchPatients(caseNumber);
					suggestions = results;
					showSuggestions = results.length > 0 && !selectedPatient;
				} catch (e) {
					suggestions = [];
				}
			}, 100);
	...
	async function handleSubmit() {
		...
		if (!sourceHandle || !destHandle || !filename) {
			submitError = 'Source and destination directories must be connected';
			return;
		}
		...
		try {
			const formData: SortFormData = {
				caseNumber,
				consentStatus: consentStatus as 'no_consent' | 'consent',
				consentType: consentStatus === 'consent' ? consentType as 'hipaa' | 'social_media' : undefined,
				procedureType,
				surgeryDate,
				imageType,
				angle,
			...
			await sortImage(sourceHandle, destHandle, filename, formData, {
				manifestId: mid ?? undefined,
				patient: (!selectedPatient && firstName && lastName) ? {
					firstName,
					lastName,
					dob: (dobYear && dobMonth && dobDay) ? `${dobYear}-${dobMonth}-${dobDay}` : undefined,
